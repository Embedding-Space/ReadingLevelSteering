...
def load_model_and_tokenizer():
    ...
    # Gemma 3 is multimodal, get layers from language model
    try:
        layers = get_model_layers(model)
        num_layers = len(layers)
    except AttributeError as e:
        # Fallback: try config
        if hasattr(model, 'language_model') and hasattr(model.language_model.config, 'num_hidden_layers'):
            num_layers = model.language_model.config.num_hidden_layers
        else:
            print(f"Could not determine layer count: {e}")
            raise

    print(f"✓ Model loaded successfully")
    print(f"  - Number of layers: {num_layers}")
    # For multimodal models, config might be in language_model
    try:
        hidden_size = model.config.hidden_size
    except AttributeError:
        hidden_size = model.language_model.config.hidden_size if hasattr(model, 'language_model') else "unknown"
...
def get_model_layers(model):
    ...
    try:
        # Standard transformer models (Qwen, Llama)
        return model.model.layers
    except AttributeError:
        pass

    # Gemma 3 multimodal: language_model.layers (directly!)
    ...
    # If we get here, we couldn't find layers
    available_attrs = dir(model) if not hasattr(model, 'model') else dir(model.model)
    raise AttributeError(f"Cannot find layers in model. Top-level attributes: {[a for a in dir(model) if not a.startswith('_')]}")
