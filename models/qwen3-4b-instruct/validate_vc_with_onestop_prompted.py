#!/usr/bin/env python3
"""
Validate V_c with PROMPTED OneStop dataset.

Hypothesis: V_c might be a generation-space vector, not a comprehension-space vector.
By adding "Please summarize the following text:" before each article, we force the
model into generation mode where complexity representations may be more active.

Compare to unprompted version to test if task context affects internal representation.
"""
...
def get_activation_at_layer(model, tokenizer, text, layer_idx):
    """Get activation vector at specified layer after processing text with task prompt."""
    # Add task instruction before text
    prompted_text = f"Please summarize the following text:\n\n{text}"

    # Tokenize
    inputs = tokenizer(prompted_text, return_tensors="pt", truncation=True, max_length=512).to(DEVICE)
...
def main():
    ...
    # Save results
    csv_path = OUTPUT_DIR / "onestop_validation_prompted_results.csv"
    ...
    ax.set_title(
        'Does Task Context Activate Complexity Representations?\n'
        'Testing with "Please summarize:" prompt prefix',
        fontsize=13,
        fontweight='bold',
        pad=15
    )
    ...
    plt.tight_layout()
    plot_path = OUTPUT_DIR / "onestop_validation_prompted_analysis.png"
