#!/usr/bin/env python3
"""
Self-consistency check: Validate last-token V_c against Wikipedia training data.

This tests whether using CONSISTENT extraction and validation methods improves
correlation. Both extraction and validation now use last-token activations only.

Hypothesis: Should show STRONGER correlation than sequence-averaged validation
(R²=0.62) because we're comparing apples to apples.
"""
...
WIKIPEDIA_PATH = SCRIPT_DIR / "../../data/wikipedia_pairs/wikipedia_pairs.json"
VC_PATH = SCRIPT_DIR / "output/complexity_vectors_last_token.pt"
OUTPUT_DIR = SCRIPT_DIR / "output"


def load_wikipedia_data():
    """Load and prepare Wikipedia articles (same ones used for extraction)."""
    print(f"\n{'='*80}")
    print("LOADING WIKIPEDIA DATASET (TRAINING DATA)")
    print(f"{'='*80}\n")

    if not WIKIPEDIA_PATH.exists():
        raise FileNotFoundError(f"Wikipedia data not found at {WIKIPEDIA_PATH}")

    with open(WIKIPEDIA_PATH) as f:
        data = json.load(f)

    # Extract pairs (simple and regular texts) - use first 20 like extraction did
    texts = []
    for i, pair in enumerate(data['pairs'][:20]):  # Match extraction script
        # Simple version
        texts.append({
    ...
    print(f"(Using first 20 pairs - same as extraction)")
...
def get_activation_at_layer(model, tokenizer, text, layer_idx):
    """Get LAST TOKEN activation vector at specified layer after processing text."""
    # Tokenize
...
def main():
    # Load Wikipedia data (same as extraction)
    df = load_wikipedia_data()

    # Load last-token V_c
    print(f"\n{'='*80}")
    print("LOADING COMPLEXITY VECTOR (LAST-TOKEN METHOD)")
    print(f"{'='*80}\n")
    ...
    # Save results
    csv_path = OUTPUT_DIR / "wikipedia_validation_results_last_token.csv"
    ...
    # Load sequence-averaged R² for comparison
    try:
        seq_avg_df = pd.read_csv(OUTPUT_DIR / "wikipedia_validation_results.csv")
        slope_seq, intercept_seq, r_seq, p_seq, std_seq = linregress(
            seq_avg_df['fk_grade'],
            seq_avg_df['dot_product']
        )
        r_squared_seq = r_seq ** 2
        print(f"Comparison to sequence-averaged validation:")
        print(f"  Sequence-averaged R²: {r_squared_seq:.4f}")
        print(f"  Last-token R²: {r_squared:.4f}")
        print(f"  Improvement: {(r_squared - r_squared_seq):.4f}")
        if r_squared > r_squared_seq:
            print(f"  ✓ Last-token method shows BETTER correlation!")
        elif r_squared < r_squared_seq:
            print(f"  ✗ Last-token method shows WORSE correlation")
        else:
            print(f"  → Same correlation (rounding)")
    except FileNotFoundError:
        print("(No sequence-averaged results for comparison)")
    ...
    ax.set_title(
        'Self-Consistency Check: Wikipedia Training Data\n'
        'Last-token extraction + last-token validation',
        fontsize=13,
        fontweight='bold',
        pad=15
    )
    ...
    ax.set_xlabel('Wikipedia Reading Level', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = OUTPUT_DIR / "wikipedia_validation_analysis_last_token.png"
