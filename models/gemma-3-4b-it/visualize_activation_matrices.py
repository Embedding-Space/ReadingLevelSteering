...
Creates two plots:
1. 2×3 grid: Simple/Regular × Layer 0/middle/final raw activations
2. 1×3 grid: Difference matrices (Regular - Simple) at each layer

Each matrix is 4096 tokens (horizontal) × hidden_dim features (vertical),
...
# Gemma has 34 layers total
LAYERS_TO_VISUALIZE = [0, 17, 33]
...
def load_model_and_tokenizer():
    ...
    # Get layers - Gemma has model.language_model.layers, Qwen has model.model.layers
    ...
    # Get hidden size - Gemma has text_config.hidden_size, Qwen has hidden_size
...
def main():
    ...
    plot_raw_matrices(simple_matrices, regular_matrices, pair['topic'], hidden_dim)
    plot_difference_matrices(simple_matrices, regular_matrices, pair['topic'], hidden_dim)
